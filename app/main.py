# app/main.py
# =============================================================================
# Conditional Autoregressive (PixelCNN/Transformer) — production entry point
# Phase-2 parity with other repos (Diffusion, DCGAN, VAE, MAF, GMM, RBM)
#
# Commands
# --------
# python -m app.main train         # train AR model, save checkpoints
# python -m app.main synth         # load latest AR checkpoint and synthesize per-class samples
# python -m app.main eval          # standardized evaluation with/without synthetic
# python -m app.main all           # train -> synth -> eval
#
# Phase-2 outputs
# ---------------
# - runs/console.txt               (clean human-readable block; via gcs_core or local)
# - runs/summary.jsonl             (append-only JSON lines; via gcs_core or local)
# - artifacts/.../summaries/*.json (pretty JSON for the aggregator; Phase-2 schema)
#
# Notes
# -----
# • Training & evaluation use images in [0,1] float32, NHWC.
# • Labels are one-hot encoded (N,K) float32.
# • This file mirrors other repos so aggregation/CI stay consistent.
# =============================================================================

from __future__ import annotations

# --- Make repo-local packages importable (autoregressive/, etc.) --------------
import sys
...
# ------------------------------------------------------------------------------
# Autoregressive pipeline & helpers
# ------------------------------------------------------------------------------
from autoregressive.pipeline import AutoregressivePipeline  # type: ignore
from autoregressive.models import build_ar_model            # type: ignore
try:
    from autoregressive.sample import save_grid_from_model as save_grid  # type: ignore
except Exception:  # pragma: no cover
    save_grid = None  # fallback preview defined below
...
def latest_ar_checkpoint(ckpt_dir: Path) -> Optional[Path]:
    """
    Pick the most appropriate AR checkpoint (newest first by preference):
      1) AR_best.weights.h5
      2) AR_last.weights.h5
      3) Latest AR_epoch_*.weights.h5
      4) Legacy: newest AR_epoch_*.h5 (no '.weights')
    """
    preferred = [ckpt_dir / "AR_best.weights.h5", ckpt_dir / "AR_last.weights.h5"]
    for p in preferred:
        if p.exists():
            return p
    epoch_ckpts = sorted(ckpt_dir.glob("AR_epoch_*.weights.h5"))
    if epoch_ckpts:
        return max(epoch_ckpts, key=lambda pp: pp.stat().st_mtime)
    legacy = sorted(ckpt_dir.glob("AR_epoch_*.h5"))
    return max(legacy, key=lambda pp: pp.stat().st_mtime) if legacy else None


# =============================================================================
# Minimal preview (only if repo helper is unavailable)
# =============================================================================
def _quick_preview_grid(
    model: tf.keras.Model, img_shape: Tuple[int, int, int], num_classes: int, out_path: Path
) -> None:
    """
    Dependency-free preview: sample one rasterized image per class and save a PNG grid.
    Assumes the model outputs per-pixel probabilities when called with (x_so_far, one-hot label).
    """
    import matplotlib.pyplot as plt  # lazy import for CLI environments
    ...
    for cls in range(num_classes):
        y = tf.keras.utils.to_categorical([cls], num_classes=num_classes).astype(np.float32)
        x = np.zeros((1, H, W, C), dtype=np.float32)
        # Simple raster Bernoulli sampling (batch=1)
        for i in range(H):
            for j in range(W):
                p = model.predict([x, y], verbose=0)[:, i, j, 0]
                x[:, i, j, 0] = (np.random.rand(1) < p).astype(np.float32)
        axes[cls].imshow(x[0, :, :, 0], cmap="gray", vmin=0.0, vmax=1.0)
        axes[cls].set_axis_off()
        axes[cls].set_title(f"C{cls}", fontsize=9)
...
def run_train(cfg: Dict) -> None:
    """Train the autoregressive model and write a small preview grid."""
    ...
    pipe = AutoregressivePipeline(cfg | {"LOG_CB": make_log_cb(Path(cfg["ARTIFACTS"]["tensorboard"]))})
    model = pipe.train(x_train=x_train01, y_train=y_train, x_val=x_val01, y_val=y_val)

    # Save a compact preview (prefer repo helper; fallback to local sampler)
    preview_path = Path(cfg["ARTIFACTS"]["summaries"]) / "train_preview.png"
    try:
        if callable(save_grid):
            save_grid(model, num_classes=num_classes, path=preview_path)
        else:
            _quick_preview_grid(model, img_shape, num_classes, preview_path)
        print(f"Saved preview grid to {preview_path}")
    except Exception:
        # Preview is non-critical; keep training output usable even if it fails.
        pass


def run_synth(cfg: Dict) -> None:
    """Load latest AR checkpoint and synthesize balanced per-class arrays to disk."""
    set_seed(cfg.get("SEED", 42))
    ensure_dirs(cfg)

    ckpt_dir = Path(cfg["ARTIFACTS"]["checkpoints"])
    ckpt = latest_ar_checkpoint(ckpt_dir)
    if ckpt is None:
        raise FileNotFoundError(f"No AR checkpoints found in {ckpt_dir}")

    # Rebuild model with config-compatible args and load weights
    model = build_ar_model(
        ...
        ff_multiplier=int(cfg.get("FF_MULT", 2)),
        learning_rate=float(cfg.get("LR", 2e-4)),
        beta_1=float(cfg.get("BETA_1", 0.5)),
    )
    model.load_weights(str(ckpt))
    print(f"Loaded AR checkpoint: {ckpt.name}")

    pipe = AutoregressivePipeline(cfg)
    x_s, y_s = pipe.synthesize(model)  # pipeline saves arrays under ARTIFACTS/synthetic
...
def run_eval(cfg: Dict, include_synth: bool) -> None:
    ...
    Phase-2 standardized evaluation:
      ...
      • Outputs:
          - runs/console.txt
          - runs/summary.jsonl
          - artifacts/autoregressive/summaries/ConditionalAutoregressive_eval_summary_seed{SEED}.json
    """
    set_seed(cfg.get("SEED", 42))
    ensure_dirs(cfg)
    ...
    record = _write_phase2_summary(
        model_name="ConditionalAutoregressive",
        seed=seed,
        data_dir=data_dir,
    ...
    out_path = Path(cfg["ARTIFACTS"]["summaries"]) / f"ConditionalAutoregressive_eval_summary_seed{seed}.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Conditional Autoregressive pipeline runner")
...
def main() -> None:
    ...
    cfg.setdefault("EVAL_EPOCHS", 20)          # evaluator CNN epochs (separate from AR train)
    ...
    cfg["ARTIFACTS"].setdefault("checkpoints", "artifacts/autoregressive/checkpoints")
    cfg["ARTIFACTS"].setdefault("synthetic",   "artifacts/autoregressive/synthetic")
    cfg["ARTIFACTS"].setdefault("summaries",   "artifacts/autoregressive/summaries")
